...
| `providers.webSearchXaiModel`      | string  | _(unset)_ | Grok model ID for xAI Responses web search when `web_search` uses the `xai` provider; defaults to `grok-4.6`, overridden by `XAI_SEARCH_MODEL`. Relays can instead be configured per-provider in `models.yml` (`providers.xai.baseUrl`/`apiKey`) or via `XAI_BASE_URL` + `XAI_API_KEY`.                                                                                                                                                  |
