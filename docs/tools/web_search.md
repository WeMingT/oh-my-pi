...
- **Provider adapters**
  ...
  - **xAI** — `packages/coding-agent/src/web/search/providers/xai.ts`
    ...
    - Querying: POSTs the Responses API with `tools: [{ type: "web_search", ... }]` and reasoning effort `low`. A custom model-registry endpoint is supported, but official xAI OAuth credentials are refused for custom endpoints.
    - Model: `providers.webSearchXaiModel` selects the Grok search model; `XAI_SEARCH_MODEL` overrides it. Defaults to `grok-4.6`. Relay/self-hosted setups can point the provider at another Responses-compatible endpoint via `models.yml` `providers.xai.baseUrl`/`apiKey` or `XAI_BASE_URL` + `XAI_API_KEY` (the registry path resolves `XAI_BASE_URL` last, and registry-less callers such as SDK custom-tool embedding honor it directly).
