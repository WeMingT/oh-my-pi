...
- Provider credit-exhaustion detection recognizes balance/billing account-state phrases and Chinese quota rejections (including 配额已耗尽), without treating load-balancer or billing-webhook failures as exhausted credits. 401/403 responses keep a diagnosed billing failure out of the generic "authorization failed" summary.
- Provider credit-exhaustion detection recognizes balance/billing account-state phrases and Chinese quota rejections (including 配额已耗尽), without treating load-balancer, billing-webhook, or transient resource-exhausted failures as exhausted credits. 401/403 responses keep a diagnosed billing failure out of the generic "authorization failed" summary.
- Provider credit-exhaustion detection recognizes balance/billing account-state phrases and Chinese quota rejections (including 配额已耗尽), without treating load-balancer, billing-webhook, transient resource-exhausted, or unrelated infrastructure exhaustion ("retry attempts exhausted") failures as exhausted credits. 401/403 responses keep a diagnosed billing failure out of the generic "authorization failed" summary.
- Provider credit-exhaustion detection recognizes balance/billing account-state phrases and Chinese quota rejections (including 配额已耗尽), without treating load-balancer, billing-webhook, transient resource-exhausted, unrelated infrastructure exhaustion ("retry attempts exhausted"), or payment-context "credit card" failures as exhausted credits. 401/403 responses keep a diagnosed billing failure out of the generic "authorization failed" summary.
- Provider credit-exhaustion detection recognizes balance/billing account-state phrases, subscription-cap rejections ("Your subscription has reached its rate limit"), and Chinese quota rejections (including 配额已耗尽), without treating load-balancer, billing-webhook, transient resource-exhausted, unrelated infrastructure exhaustion ("retry attempts exhausted"), or payment-context "credit card" failures as exhausted credits. 401/403 responses keep a diagnosed billing failure out of the generic "authorization failed" summary.
- Provider credit-exhaustion detection is delegated entirely to pi-ai's shared account-quota predicate, which recognizes balance/billing account-state phrases, subscription-cap rejections ("Your subscription has reached its rate limit"), and Chinese quota rejections (including 配额已耗尽), without treating limit-configuration wording ("usage limit configuration is invalid"), load-balancer, billing-webhook, transient resource-exhausted, unrelated infrastructure exhaustion ("retry attempts exhausted"), or payment-context "credit card" failures as exhausted credits. 401/403 responses keep a diagnosed billing failure out of the generic "authorization failed" summary.
- Provider credit-exhaustion detection is delegated entirely to pi-ai's shared account-quota predicate, which recognizes balance/billing account-state phrases, subscription-cap rejections ("Your subscription has reached its rate limit"), copula-separated quota exhaustion ("quota has been exhausted", "you have exceeded your quota"), and Chinese quota rejections (including 配额已耗尽), without treating limit- or subscription-configuration wording ("usage limit configuration is invalid"), load-balancer, billing-webhook, transient resource-exhausted, unrelated infrastructure exhaustion ("retry attempts exhausted"), or payment-context "credit card" failures as exhausted credits. 401/403 responses keep a diagnosed billing failure out of the generic "authorization failed" summary.
- Provider credit-exhaustion detection is delegated entirely to pi-ai's shared account-quota predicate, which recognizes balance/billing account-state phrases (including reverse "Insufficient balance" wording and the structured G1 credits-balance reason), subscription-cap rejections ("Your subscription has reached its rate limit"), copula-separated quota exhaustion ("quota has been exhausted", "you have exceeded your quota"), and Chinese quota rejections (including 额度不足 and 配额已耗尽), without treating limit- or subscription-configuration wording ("usage limit configuration is invalid"), load-balancer, billing-webhook, transient resource-exhausted, unrelated infrastructure exhaustion ("retry attempts exhausted"), or payment-context "credit card" failures as exhausted credits. 401/403 responses keep a diagnosed billing failure out of the generic "authorization failed" summary.
- Provider credit-exhaustion detection is delegated entirely to pi-ai's shared account-quota predicate, which recognizes balance/billing account-state phrases (including reverse "Insufficient balance" wording and the structured G1 credits-balance reason), subscription-cap rejections ("Your subscription has reached its rate limit"), copula-separated quota exhaustion ("quota has been exhausted", "you have exceeded your quota"), and Chinese quota rejections (including 额度不足 and 配额已耗尽), without treating negated states ("usage limit has not been reached"), limit- or subscription-configuration wording ("usage limit configuration is invalid"), load-balancer, billing-webhook, transient resource-exhausted, unrelated infrastructure exhaustion ("retry attempts exhausted"), or payment-context "credit card" failures as exhausted credits. 401/403 responses keep a diagnosed billing failure out of the generic "authorization failed" summary.
...
- Web search recognizes known balance/billing messages in plain-text and JSON errors — including envelopes where an unrelated `error` field would otherwise shadow a recognized top-level `message` — and preserves billing diagnostics on 401/403 responses instead of reporting an authorization failure.
- Web search recognizes known balance/billing messages in plain-text and JSON errors — including envelopes where an unrelated `error` field would otherwise shadow a recognized top-level `message` — preserves billing diagnostics on 401/403 responses instead of reporting an authorization failure, and keeps authorization summaries on 401/403 when only the broad legacy heuristic matches (e.g. "insufficient authentication scope").
- Web search recognizes known balance/billing messages in plain-text and JSON errors — including envelopes where an unrelated `error` field would otherwise shadow a recognized top-level `message` — preserves billing diagnostics on 401/403 responses (exact billing aliases and explicit credit-exhaustion wording) instead of reporting an authorization failure, and keeps authorization summaries on 401/403 when only ambiguous wording such as "insufficient authentication scope" matches.
- GitHub Copilot model-policy 403s (plan, model policy, org restriction) no longer delete stored credentials, so the provider stays listed in `/model` after a per-model access denial instead of disappearing until the next `/login` ([#11280](https://github.com/can1357/oh-my-pi/pull/11280) by [@H4vC](https://github.com/H4vC)).
- Read error and preview rendering now sanitizes tabs and Windows-style CRLF (e.g. ssh host-key failures, tab-indented fetched content) so raw output can no longer tear the result frame.
