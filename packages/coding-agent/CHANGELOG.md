...
- xAI search answer extraction honors explicit `commentary`/`final_answer` phases, treats unrecognized or empty phase values as unphased, prefers phased messages over the aggregate `output_text`, keeps an explicit final answer followed by an empty final item, and never restores the aggregate (which mixes commentary in) once commentary is tagged; unphased relay messages retain the final message and substantive earlier content (citations or long text).
